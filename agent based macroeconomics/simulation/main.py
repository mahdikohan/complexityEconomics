        ...
        if huid != None:
            for employee in self.employees:
                if employee.get_huid() == huid:
                    # ????
                    print(f'Before remove: {len(self.employees)}')
                    self.employees.remove(employee)
                    print(f'After remove: {len(self.employees)}')
                    print('++++++++++++++++++++++++++++++++++++')

        elif huid == None:
            if self.next_month_fire != '':
                self.disconnect_employee(huid= self.next_month_fire.get_huid())
                self.employees_Cap = self.employees_Cap - 1
...
if __name__ == "__main__":
    ...
    for _ in range(num_month):
        ...
        for f in firms:
            for h in households:
                sample_firms_group = random.sample(firms,random.randrange(1,ife))
